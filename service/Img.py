...
def cropImageByScale(filePath: str, width: int, height: int):
    ...
    if origin_w / origin_h > width / height:
        new_w = origin_h * width // height
        new_h = origin_h
    else:
        new_w = origin_w
        new_h = origin_w * height // width
    img = img[(origin_h - new_h) // 2:(origin_h + new_h) // 2, (origin_w - new_w) // 2:(origin_w + new_w) // 2]
    cv2.imwrite(filePath, img)
    return True
